...
export default class ChimeSdkWrapper implements DeviceChangeObserver {
  ...
  getBaseUrl = 'https://cc5qph0pl0.execute-api.ap-southeast-1.amazonaws.com/Prod/';
  getMessagingWssUrl = 'wss://oa1dvbwwki.execute-api.ap-southeast-1.amazonaws.com/Prod';
  ...
  createRoom = async (
    ...
    if (!title || !name || !region) {
      this.logError(new Error(`title=${title} name=${name} region=${region} must exist`));
      return;
    }

    const response = await fetch(
      `${this.getBaseUrl}join?title=${encodeURIComponent(title)}&name=${encodeURIComponent(
        name,
  ...
  initializeMeetingSession = async (configuration: MeetingSessionConfiguration): Promise<void> => {
    ...
    this.videoInputDevices = [];
    (await this.audioVideo?.listVideoInputDevices()).forEach((mediaDeviceInfo: MediaDeviceInfo) => {
        ...
        this.audioVideo?.realtimeSubscribeToVolumeIndicator(
          ...
          async (
            ...
            if (this.title && attendeeId && !this.roster[attendeeId].name) {
              const response = await fetch(
                `${this.getBaseUrl}attendee?title=${encodeURIComponent(
                  this.title,
  ...
  joinRoomMessaging = async (): Promise<void> => {
    ...
    const messagingUrl = `${this.getMessagingWssUrl}?MeetingId=${this.configuration.meetingId}&AttendeeId=${this.configuration.credentials?.attendeeId}&JoinToken=${this.configuration.credentials?.joinToken}`;
  ...
  leaveRoom = async (end: boolean): Promise<void> => {
    ...
    try {
      if (end && this.title) {
        await fetch(`${this.getBaseUrl}end?title=${encodeURIComponent(this.title)}`, {
          method: 'POST',
        });
      }
    } catch (error) {
  ...
  subscribeToDevicesUpdated = (callback: (fullDeviceInfo: FullDeviceInfoType) => void) => {
    this.devicesUpdatedCallbacks.push(callback);
  };

  unsubscribeFromDevicesUpdated = (callback: (fullDeviceInfo: FullDeviceInfoType) => void) => {
